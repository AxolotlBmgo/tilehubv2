...
local function C_112()
	...
	local function babftautofarmstart()
		...
		if game.Workspace:WaitForChild("babftautofarmondeath").Value == false then
			...
			-- Roblox script to print "you died" when the player dies

			local stage0Value = otherData:WaitForChild("Stage0")
			local stage1Value = otherData:WaitForChild("Stage1")
			local stage2Value = otherData:WaitForChild("Stage2")
			local stage3Value = otherData:WaitForChild("Stage3")
			local stage4Value = otherData:WaitForChild("Stage4")
			local stage5Value = otherData:WaitForChild("Stage5")
			local stage6Value = otherData:WaitForChild("Stage6")
			local stage7Value = otherData:WaitForChild("Stage7")
			local stage8Value = otherData:WaitForChild("Stage8")
			local stage9Value = otherData:WaitForChild("Stage9")




			-- Function to connect to character humanoid
			local function onCharacterAdded(character)
				local humanoid = character:WaitForChild("Humanoid")
				humanoid.Died:Connect(function()
					-- Reset stage values
					stage0Value.Value = ""
					-- Other stage values reset...

					-- Check if the player should be teleported to stage 0
					if stage0Value.Value == "" then
						teleportPlayerToStage(0)
					end
				end)
			end

			-- Connect when character is added
		...
		local function setupStageListener(stage, nextStage, customDelayTeleportBeforeNext, extraTeleportAfterNext)
		...
		local function setupStage9Listener()
			local stage9Value = otherData:WaitForChild("Stage9")
			if not stage9Value then return end

			if stage9Value.Value ~= "" then
				teleportPlayerToCustomPosition(stage9CustomPosition)
			else
				stage9Value.Changed:Connect(function(newValue)
					if newValue ~= "" then
						teleportPlayerToCustomPosition(stage9CustomPosition)
					end
				end)
		...
		for stage = 0, 9 do
			...
			local extraTeleportAfterNext = (stage == 9)  -- after stage9, teleport extra to stage9CustomPosition
			setupStageListener(stage, nextStage, customDelayTeleportBeforeNext, extraTeleportAfterNext)
		end
